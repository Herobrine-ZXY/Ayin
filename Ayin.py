...
def load_api_keys():
    """从文件加载API密钥"""
    global CEPHALON_API_KEY, VIRUSTOTAL_API_KEY
    ...
    try:
        ...
        with open(API_KEYS_FILE, 'r', encoding='utf-8') as f:
            ...
            if len(lines) >= 1:
                # 第一行：Cephalon API密钥
                if "=" in lines[0]:
                    CEPHALON_API_KEY = lines[0].split("=")[1].strip()

            if len(lines) >= 2:
                # 第二行：VirusTotal API密钥
                if "=" in lines[1]:
                    VIRUSTOTAL_API_KEY = lines[1].split("=")[1].strip()
...
def load_conversation_history() -> bool:
    """加载对话历史"""
    global messages
    try:
...
def save_conversation_history() -> bool:
    """保存对话历史"""
    try:
...
def clean_old_files(days_to_keep: int = 20) -> bool:
    """清理旧文件"""
    try:
        ...
        with vt.Client(VIRUSTOTAL_API_KEY, timeout=15) as client:
            ...
            with open(file_path, "rb") as f:
                ...
                result_text = (
                    ...
                    f"未检测引擎数: {undetected}/{total}\n"
                    f"超时引擎数: {timeout}/{total}\n"
...
def generate_image_from_text():
    """调用ComfyUI API生成图像"""
    if not CEPHALON_API_KEY:
        log_error("未配置Cephalon API密钥")
        return
    ...
    threading.Thread(
        ...
        args=(prompt, width, height, guidance, steps, seed_value, start_time),
        daemon=True
    ).start()


def perform_image_generation(prompt: str, width: int, height: int, guidance: float,
                             steps: int, seed: int, start_time: float):
    """执行图像生成任务"""
    try:
        if not is_internet_available():
            log_error("无法连接到互联网，请检查网络连接")
...
def fetch_assistant_reply_stream(user_input: str):
    """流式获取助手回复"""
    global openai_client, messages, output_box
    if not CEPHALON_API_KEY:
        log_error("未配置Cephalon API密钥")
        return
...
def handle_api_error(e: Exception, user_input: str):
    """处理API错误"""
...
5. API密钥
- API密钥存储在同目录下的api_keys.txt中
- 第一行: cephalon_api_key
- 第二行: virustotal_api_key
...
def toggle_image_panel():
    """切换图像面板显示状态"""
    global image_panel, image_button
...
# 输出框框架
output_frame = Frame(main_frame, bg="#2d极2d2d")
...
Label(image_panel, text="x", bg="#333333", fg="white").grid(row=2, column=2)
...
virus_button.pack(side="left", padx=5, pady=5)
...
# API密钥状态
api_status = "API密钥: " + ("已加载" if CEPHALON_API_KEY and VIRUSTOTAL_API_KEY else "未配置")
api_label = Label(
    status_frame, text=api_status,
    bg="#222222", fg="#aaaaaa", font=("Microsoft YaHei", 9)
)
api_label.grid(row=0, column=2, padx=10, sticky="e")
